...
/// Migrate existing SQLite memory entries to markdown files.
pub async fn memory_migrate(
    ...
    let entries = db.get_all_entries().map_err(|e| {
        (
            ErrorCode::InternalError,
            format!("Failed to load entries: {e}"),
        )
    ...
    // Write sentinel file so tooling can detect completed migrations.
    let sentinel = format!(
    ...
    store
        .write("migrated/.migration_complete", &sentinel)
        .await
        .map_err(|e| {
            (
                ErrorCode::InternalError,
                format!("Failed to write sentinel: {e}"),
            )
        })?;
