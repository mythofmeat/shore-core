...
#[tokio::test]
async fn memory_migrate_no_db_returns_no_db() {
    let tmp = TempDir::new().unwrap();
    let (engine, ctx, _rx) = make_ctx(&tmp);

    let result = memory_migrate(&engine, &ctx).await.unwrap();
    assert_eq!(result["status"], "no_db");
    assert_eq!(result["migrated"], 0);
}

#[tokio::test]
async fn memory_migrate_empty_db_returns_empty() {
    let tmp = TempDir::new().unwrap();
    let (engine, ctx, _rx) = make_ctx(&tmp);
...
#[tokio::test]
async fn memory_migrate_writes_markdown_and_sentinel() {
    let tmp = TempDir::new().unwrap();
    let (engine, ctx, _rx) = make_ctx(&tmp);
    ...
    db.create_entry(&crate::memory::db::Entry {
        id: "20250101_120000_0".into(),
        memory_type: "episodic".into(),
        source: "test".into(),
        reason: "migrate test".into(),
        status: "active".into(),
        confidence: 0.9,
        summary_text: "Ren likes jazz and plays saxophone.".into(),
        topic_tags: "music,preferences".into(),
        topic_key: "music".into(),
        start_timestamp: "2025-01-01T12:00:00Z".into(),
        end_timestamp: "2025-01-01T12:00:00Z".into(),
        message_count: 5,
        source_entry_ids: String::new(),
        related_entry_ids: String::new(),
        superseded_by: String::new(),
    ...
    let memories_dir = ctx.data_dir.join("TestChar").join("memories").join("migrated");
    let md_path = memories_dir.join("music_20250101_120000_0.md");
    assert!(md_path.exists(), "migrated markdown file should exist");
    let content = std::fs::read_to_string(&md_path).unwrap();
    assert!(content.contains("Ren likes jazz"));

    // Verify sentinel was written.
    let sentinel_path = memories_dir.join(".migration_complete");
    assert!(sentinel_path.exists(), "sentinel file should exist");
    let sentinel = std::fs::read_to_string(&sentinel_path).unwrap();
    assert!(sentinel.contains("Migration completed at"));
    assert!(sentinel.contains("Migrated 1 entries"));
}
