...
- **OpenRouter intermittently drops prompt cache hits (~22-33% miss rate).** Systematic testing (2026-04-09) confirmed this is independent of: API format (OpenAI vs Anthropic), HTTP headers, inter-message delay (4s/8s/15s), breakpoint strategy (sliding vs system-only), and content normalization. The pattern suggests OpenRouter load-balances across multiple Anthropic backends with independent prompt caches — each backend switch is a full miss. SillyTavern achieves <1% misses through the same OpenRouter, likely via provider pinning (`provider.order = ["Anthropic"]`). **Provider pinning is the suspected fix but could not be confirmed (credits exhausted during testing).** This is the highest-priority retest. See `docs/PROMPT_CACHING.md` for the full experiment matrix.
