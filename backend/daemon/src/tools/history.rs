...
pub fn tool_defs() -> Vec<ToolDef> {
    vec![ToolDef {
        name: "search_history",
        description: "Search your own conversation history, including compacted older segments and the active conversation window. Use this when the user asks what you discussed before, when you need to verify a past exchange, or when the answer may be in the transcript rather than in curated memory files. Returns matching messages with role, timestamp, source, and excerpt.",
        parameters: json!({
            "type": "object",
            "properties": {
                "query": {
