#!/usr/bin/env bun
/**
 * Tier 1 parity check: delete command + restart-persistence.
 *
 * Replays `parity-traces/delete.jsonl` against the TS daemon. Same
 * concurrent-emission batching as parity-check-edit.ts. Restart phase
 * asserts the deleted messages are absent from the persisted history.
 *
 * Usage:
 *   bun scripts/parity-check-delete.ts [--fixture <dir>] [--baseline <path>] [<daemon>]
 */
...
let baselinePath = "parity-traces/delete.jsonl";
...
const env = buildDaemonEnv({ configDir, dataDir, prefix: "shore-delete-parity-" });
