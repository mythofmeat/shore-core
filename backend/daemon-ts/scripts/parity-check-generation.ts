...
try {
  const rust = await runScenario("rust", [args.rust], resolvePath(args.fixture), proxy.baseUrl);
  const ts = await runScenario("ts", tsCmd, resolvePath(args.fixture), proxy.baseUrl);
