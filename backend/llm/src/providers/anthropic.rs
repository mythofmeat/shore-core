...
/// Find the recent user message that should advance a Claude message cache
/// breakpoint. Tool-result-only user messages count here: a tool loop resumes
/// from the newest result block just as a regular turn resumes from the newest
/// user prompt.
fn find_user_boundary(messages: &[Value], depth: usize) -> Option<usize> {
    messages
        .iter()
        .enumerate()
        .rev()
...
///
/// Each depth value N means "N user messages back from the end". Claude's
/// growing-conversation cache recipe marks the newest user-side boundary so a
/// tool loop can read the user prompt that asked for the tool and then advance
/// the breakpoint onto each completed `tool_result` continuation.
fn resolve_depth_turns(messages: &[Value], depths: &[u32]) -> Vec<usize> {
    depths
        .iter()
        .filter_map(|depth| find_user_boundary(messages, *depth as usize))
        .collect()
}
...
mod tests {
    ...
    fn test_apply_cache_control_depth_turns() {
        ...
        // depth=2: the third-most-recent user message is turn 2 at index 2.
        assert_eq!(p.msg_breakpoints.len(), 1);
        assert_eq!(p.msg_breakpoints[0], 2);
    }
    ...
    #[test]
    fn test_breakpoint_content_tracks_final_user_message() {
        // Claude's growing-conversation cache recipe advances the message
        // breakpoint onto the latest user-side boundary. That includes the
        // active final user prompt that can lead into a tool loop.
        let system = build_system_with_memory_index();
        for turn_count in 1..=10 {
            ...
            let (_, _, p) = apply_cache_control(&msgs, &system, "1h", &[0, 1], &[0]);
            assert!(
                p.msg_breakpoints.contains(&last_idx),
                "turn {turn_count}: latest user message {last_idx} should advance the breakpoint"
            );
        }
    }

    #[test]
    fn test_default_depths_single_user_marks_the_user_boundary() {
        let system = build_system_with_memory_index();
        let msgs = build_conversation(1);
        let (_, _, p) = apply_cache_control(&msgs, &system, "1h", &[0, 1], &[-1]);

        assert_eq!(p.msg_breakpoints, vec![0]);
        assert_eq!(p.sys_breakpoints, vec![1]);
    }
    ...
    #[test]
    fn first_tool_loop_continuation_reuses_active_user_boundary() {
        let system = build_system_with_memory_index();
        let before_tool = vec![
        ...
        let (_, _, pre_tool) = apply_cache_control(&before_tool, &system, "1h", &[0, 1], &[-1]);
        assert_eq!(
            pre_tool.msg_breakpoints,
            vec![2, 4],
            "the warm pre-tool request should cache the two latest user boundaries"
        );

        let in_tool_loop = vec![
            before_tool[0].clone(),
            before_tool[1].clone(),
            before_tool[2].clone(),
            before_tool[3].clone(),
        ...
        let (result, _, placement) =
            apply_cache_control(&in_tool_loop, &system, "1h", &[0, 1], &[-1]);

        assert_eq!(
            placement.msg_breakpoints,
            vec![4, 6],
            "tool-loop placement should keep the active user boundary and add the tool_result"
        );
        assert!(
            result[4]["content"]
                .as_array()
            ...
            "active user prompt should remain a warm cache breakpoint"
        );
        assert!(
            result[6]["content"]
                .as_array()
                ...
                .iter()
                .any(|block| block.get("cache_control").is_some()),
            "completed tool_result should become a cache breakpoint"
        );
    }

    fn first_tool_loop_messages(assistant_blocks: Value) -> Vec<Value> {
        vec![
            json!({"role": "user", "content": "first question"}),
            json!({"role": "assistant", "content": "first answer"}),
            json!({"role": "user", "content": "earlier question"}),
            json!({"role": "assistant", "content": "earlier answer"}),
            json!({"role": "user", "content": "please inspect memory"}),
            json!({"role": "assistant", "content": assistant_blocks}),
            json!({"role": "user", "content": [
                {"type": "tool_result", "tool_use_id": "t1", "content": "first result"}
            ]}),
        ]
    ...
    #[test]
    fn adaptive_tool_loop_with_bare_tool_use_keeps_user_boundaries() {
        let mut request = make_request(
            first_tool_loop_messages(json!([
                {"type": "tool_use", "id": "t1", "name": "read", "input": {"path": "memory/a.md"}}
            ])),
            Some(build_system_with_memory_index()),
        );
        ...
        assert!(
            message_has_cache_control(&messages[4]),
            "adaptive tool-loop continuation should retain the user prompt breakpoint"
        );
        assert!(
            message_has_cache_control(&messages[6]),
            "adaptive tool-loop continuation should cache its tool_result boundary"
        );
    }

    #[test]
    fn adaptive_tool_loop_with_replay_thinking_keeps_user_boundaries() {
        let mut request = make_request(
            first_tool_loop_messages(json!([
                {"type": "thinking", "thinking": "Need memory.", "signature": "sig_1"},
                {"type": "tool_use", "id": "t1", "name": "read", "input": {"path": "memory/a.md"}}
            ])),
            Some(build_system_with_memory_index()),
        );
        ...
        assert!(
            message_has_cache_control(&messages[4]),
            "adaptive thinking replay should retain the user prompt breakpoint"
        );
        assert!(
            message_has_cache_control(&messages[6]),
            "adaptive thinking replay should cache its tool_result boundary"
        );
    }
    ...
    #[test]
    fn test_find_user_boundary_empty_messages() {
        assert_eq!(find_user_boundary(&[], 2), None);
    }

    #[test]
    fn test_find_user_boundary_single_user_message() {
        let msgs = vec![json!({"role": "user", "content": "hi"})];
        assert_eq!(find_user_boundary(&msgs, 0), Some(0));
        assert_eq!(find_user_boundary(&msgs, 1), None);
        assert_eq!(find_user_boundary(&msgs, 2), None);
    }

    #[test]
    fn test_find_user_boundary_depth_zero_uses_latest_user() {
        let msgs = vec![
            json!({"role": "user", "content": "q1"}),
            json!({"role": "assistant", "content": "a1"}),
            json!({"role": "user", "content": "q2"}),
        ];
        assert_eq!(find_user_boundary(&msgs, 0), Some(2));
        assert_eq!(find_user_boundary(&msgs, 1), Some(0));
    }

    #[test]
    fn test_find_user_boundary_counts_tool_result_messages() {
        let msgs = vec![
            json!({"role": "user", "content": "first question"}),
        ...
        assert_eq!(find_user_boundary(&msgs, 0), Some(6));
        assert_eq!(find_user_boundary(&msgs, 1), Some(4));
        assert_eq!(find_user_boundary(&msgs, 2), Some(2));
    }

    #[test]
    fn test_find_user_boundary_exact_depth() {
        let msgs = vec![
            json!({"role": "user", "content": "q1"}),
            json!({"role": "assistant", "content": "a1"}),
            json!({"role": "user", "content": "q2"}),
        ...
        assert_eq!(find_user_boundary(&msgs, 2), Some(0));
        assert_eq!(find_user_boundary(&msgs, 1), Some(2));
    }

    #[test]
    fn test_find_user_boundary_only_tool_result_users() {
        let msgs = vec![
