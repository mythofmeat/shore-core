...
fn handle_content_block_start(data: &str, state: &mut StreamState) -> Option<String> {
    ...
    match block_type {
        ...
        "redacted_thinking" => {
            ...
            // OpenRouter's `openrouter.reasoning:`-prefixed ones, which are
            // how OpenRouter relays signed thinking content back. They were
            // previously filtered as "bogus duplicates", but that strips the
            // reasoning data the next-turn cache prefix needs (and the model
            // may need on subsequent turns), busting the cache through every
...
mod tests {
    ...
    #[test]
    fn test_content_block_start_openrouter_redacted_preserved() {
        // `openrouter.reasoning:`-prefixed redacted_thinking blocks carry the
        // signed reasoning content OpenRouter relays back for Anthropic
        // models. Filtering them strips the reasoning data the next-turn
        // cache prefix needs, busting the cache through every tool-loop
        // continuation. The adapter must keep them verbatim.
        let mut state = StreamState::new("claude-test");
        let data = json!({
            "index": 0,
            "content_block": {
                "type": "redacted_thinking",
                "data": "openrouter.reasoning: signed reasoning payload"
            }
        })
        .to_string();
        ...
        assert!(result.is_none());
        assert_eq!(
            state.redacted_thinking_blocks.get(&0).map(String::as_str),
            Some("openrouter.reasoning: signed reasoning payload"),
            "openrouter.reasoning: blocks must be preserved for cache continuity"
        );
    }
