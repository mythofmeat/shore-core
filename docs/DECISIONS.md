...
SQLite reads.
- Not wired into the daemon command system; one-off scripts don't bloat the
  permanent binary surface.
...
separate from new AI-curated files. A standalone script is cheaper than a
permanent command that sits in the dispatch table forever.
