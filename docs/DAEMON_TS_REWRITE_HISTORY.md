...
preserved as-is for reference: the why, the architecture, the
constraints, the phased plan with per-phase done notes, the audit
findings, the per-item resolutions, and the porting wisdom that took
multiple attempts in Rust.
...
- [x] **Wire autonomous compaction (done, 2026-05-24).** Mirror of
  ...
  - Caveat: `RealCompactionLlm.summarize` still ignores `cachedRequest`
    ...
    path. *(Resolved 2026-05-25: cached request is now read at
    `autonomy/inline_compaction.ts:112` and passed through
    `runCompaction`.)*
