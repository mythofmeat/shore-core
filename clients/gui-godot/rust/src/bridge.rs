...
use shore_swp_client::conn_manager::{ConnCommand, ConnEvent, spawn_connection};
use tokio::sync::mpsc;

/// Godot node that bridges shore-swp-client (SWP) into the scene tree via signals.
///
        ...
        for event in events {
            match event {
                ConnEvent::Connected {
                    ...
                    let char_names: PackedStringArray =
                        characters.iter().map(|c| GString::from(&c.name)).collect();
...
impl ShoreBridge {
    ...
    fn connected(
        server_name: GString,
        characters: Array<GString>,
        history_json: GString,
        config_json: GString,
    );
    ...
    fn send_command(&self, name: GString, args_json: GString) {
        ...
        let args: serde_json::Value =
            serde_json::from_str(&args_json.to_string()).unwrap_or(serde_json::json!({}));
    ...
    fn dispatch_server_message(&mut self, msg: ServerMessage) {
        match msg {
            ...
            ServerMessage::StreamEnd(e) => {
                let meta_json = serde_json::to_string(&e.metadata).unwrap_or_else(|_| "{}".into());
                self.base_mut().emit_signal(
                    "stream_end",
            ...
            ServerMessage::Phase(p) => {
                ...
                self.base_mut()
                    .emit_signal("phase_changed", &[p.phase.to_variant(), model.to_variant()]);
            }
            ServerMessage::ToolCall(t) => {
                let input_json = serde_json::to_string(&t.input).unwrap_or_else(|_| "{}".into());
                self.base_mut().emit_signal(
                    "tool_call",
                    &[
            ...
            ServerMessage::NewMessage(m) => {
                let json = serde_json::to_string(&m.message).unwrap_or_else(|_| "{}".into());
                self.base_mut()
                    .emit_signal("message_received", &[json.to_variant()]);
            }
            ServerMessage::CommandOutput(co) => {
                let data_json = serde_json::to_string(&co.data).unwrap_or_else(|_| "{}".into());
                self.base_mut().emit_signal(
                    "command_output",
                    &[
