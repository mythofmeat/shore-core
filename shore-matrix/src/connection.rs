...
/// Events sent from the daemon connection task to the bridge loop.
#[derive(Debug)]
#[allow(dead_code)]
pub enum ConnEvent {
    Connected {
...
/// Commands sent from the bridge loop to the daemon connection task.
#[derive(Debug)]
#[allow(dead_code)]
...
/// Spawn the daemon connection manager.
///
                            ...
                            match msg {
                                ...
                                Ok(msg) => {
                                    if event_tx.send(ConnEvent::Message(msg)).await.is_err() {
                                        return; // Bridge loop dropped receiver
                                    }
                                }
                                Err(_) => {
                                    let _ = event_tx.send(ConnEvent::Disconnected(
