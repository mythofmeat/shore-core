...
/// Optional human-like pause before the character's reply begins streaming, so
/// chatting feels less like talking to an instant oracle. The base delay scales
/// with how long the user was silent before their message (rapid back-and-forth
/// → short delay; a message after hours → a long one), is jittered so it is
/// never exactly predictable, and is bounded by `min`/`max`. Disabled by default.
///
/// See ARCHITECTURE.md "Response delay" for the runtime model. When the delay
/// reaches `notify_after`, the character is told it kept the user waiting (so it
/// can own the lateness). A held reply is persisted and survives a daemon
/// restart — it is generated by the autonomy tick when its deadline arrives.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(deny_unknown_fields)]
pub struct ResponseDelayConfig {
    /// Master switch. When false (the default) replies stream as soon as they
    /// are ready.
    #[serde(default)]
    pub enabled: bool,

    /// Floor: the shortest delay ever applied, even during the most rapid
    /// back-and-forth. Default: 2s.
    #[serde(default = "default_response_delay_min")]
    pub min: ConfigDuration,

    /// Hard ceiling: no reply is ever held longer than this, however cold the
    /// conversation. Default: 1h.
    #[serde(default = "default_response_delay_max")]
    pub max: ConfigDuration,

    /// Fraction of the gap since the previous user message used as the base
    /// delay before jitter and clamping. Larger values mean longer waits after
    /// a silence. Default: 0.25 (≈2h of silence → a ~30m delay).
    #[serde(default = "default_response_delay_scale")]
    pub scale: f64,

    /// Random spread applied to the base delay, as a fraction in `[0, 1]`. 0.3
    /// means the actual delay varies ±30% around the base, so the user never
    /// knows exactly when the reply will land. Default: 0.3.
    #[serde(default = "default_response_delay_jitter")]
    pub jitter: f64,

    /// Once the computed delay reaches this, the character is told how long it
    /// kept the user waiting (an inline-system note it can acknowledge). Below
    /// it the pause is silent. Set larger than `max` to disable the note
    /// entirely. Default: 30m.
    #[serde(default = "default_response_delay_notify_after")]
    pub notify_after: ConfigDuration,
}

serde_default!(default_response_delay_min -> ConfigDuration { ConfigDuration::from_secs(2) });
serde_default!(default_response_delay_max -> ConfigDuration { ConfigDuration::from_secs(3600) });
serde_default!(default_response_delay_scale -> f64 { 0.25 });
serde_default!(default_response_delay_jitter -> f64 { 0.3 });
serde_default!(default_response_delay_notify_after -> ConfigDuration { ConfigDuration::from_secs(1800) });

impl Default for ResponseDelayConfig {
    fn default() -> Self {
        Self {
            enabled: false,
            min: default_response_delay_min(),
            max: default_response_delay_max(),
            scale: default_response_delay_scale(),
            jitter: default_response_delay_jitter(),
            notify_after: default_response_delay_notify_after(),
        }
    }
}

impl ResponseDelayConfig {
    /// Reject nonsensical bounds at config-load time so the daemon refuses to
    /// start rather than silently clamping surprising values. A disabled config
    /// is always valid.
    pub fn validate(&self) -> Result<(), String> {
        if !self.enabled {
            return Ok(());
        }
        if self.min.as_duration() > self.max.as_duration() {
            return Err(format!(
                "behavior.response_delay.min ({}s) must not exceed max ({}s)",
                self.min.as_secs(),
                self.max.as_secs()
            ));
        }
        if !self.scale.is_finite() || self.scale < 0.0 {
            return Err(format!(
                "behavior.response_delay.scale ({}) must be a finite, non-negative number",
                self.scale
            ));
        }
        if !self.jitter.is_finite() || !(0.0..=1.0).contains(&self.jitter) {
            return Err(format!(
                "behavior.response_delay.jitter ({}) must be between 0.0 and 1.0",
                self.jitter
            ));
        }
        Ok(())
