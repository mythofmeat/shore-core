...
fn validate_config(app: &AppConfig, catalog: &ModelCatalog) -> Result<(), ConfigError> {
    ...
    validate_model_ref(catalog, "defaults.heartbeat", app.defaults.heartbeat.as_deref())?;
    validate_model_ref(catalog, "defaults.dreaming", app.defaults.dreaming.as_deref())?;
...
"#,
        ...
        let msg = err.to_string();
        assert!(msg.contains("defaults.heartbeat"), "{msg}");
...
"#,
        ...
        let err = load_config(Some(&tmp.path().join("config.toml"))).unwrap_err();
        assert!(err.to_string().contains("defaults.dreaming"));
    }
