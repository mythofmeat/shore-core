#!/usr/bin/env bash
#
# Test: Default cache config, forced to Google Vertex via OpenRouter.
#
set -euo pipefail
source "$(dirname "$0")/harness.sh"

harness_init "vertex-defaults"

# Use hardcoded defaults (depth=[1,2] pinned=[-1]).
CACHE_DEPTH_TURNS=""
CACHE_PINNED_POSITION=""
REASONING_EFFORT="high"
OPENROUTER_PROVIDER='{ order = ["Google"] }'
